...
class class_drone_controller_display:
    ...
    def update_switches(self):
        ...
        for i in range(len(self.info.arr_switch)):
            #switch_label = tk.Label(self.switch_frame, text=f"Switch {i + 1}: {'ON' if self.info.arr_switch[i] else 'OFF'}", manchor="w", bg="#404040", fg="white", font=("Arial", 8))  # White text color
            switch_label = tk.Label(self.switch_frame,
                                    text=f"Switch {i + 1}: {self.info.arr_switch[i]}", manchor="w",
                                    bg="#404040", fg="white", font=("Arial", 8))  # White text color
